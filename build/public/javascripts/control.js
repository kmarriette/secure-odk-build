...
;(function($)
{
    ...
    var refreshFromProperties = function($this, type, options, properties)
    {
        ...
        console.log(properties.sens)
        if (typeof(properties.sens) != "undefined"){
          var sensValues=""
          if(properties.sens.level){
              sensValues+="Sensitivity Level: "+properties.sens.level+"; ";
          }

        	if(properties.sens.securitymechanisms.encryption){
        		sensValues+="Encryption:"+properties.sens.securitymechanisms.encryption
        	}
        	if(properties.sens.securitymechanisms.masking){
        		sensValues+=" Masking:"+properties.sens.securitymechanisms.masking
        	}
        	$info.children('.sensName').text(sensValues);
        }

        //console.log("Properties Sens"+JSON.stringify(properties.sens));
        //$info.children('.sensName').text(properties);
        if (type == 'group')
        {
    ...
    {
    	 console.log(TAG+"selected me"+JSON.stringify(type));
        if (selectMany === true)
        {
            if ($initiator == null)
    ...
    $.fn.odkControl = function(type, options, defaultProperties)
    {
        ...
        return this.each(function()
        {
            ...
            _.each(properties, function(property, name)
            {
            	 //console.log("property="+JSON.stringify(name));
                if (name === 'metadata') return;
