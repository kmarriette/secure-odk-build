...
;(function($)
{
    ...
    var getDataRepresentation = odkmaker.data.extractOne = function($control)
    {
        ...
        {
        	//console.log(TAG +"property value ="+ JSON.stringify(property.value)+" name ="+ JSON.stringify(name));

            data[name] = property.value;
        });
        data.metadata = properties.metadata;
        data.sens = properties.sens;
    ...
    {
        //console.log("Data Extracting pdf ="+JSON.stringify(odkmaker.data.currentForm.pdf))
        console.log("Current Form ="+JSON.stringify(odkmaker.data.currentForm))
        var htitle = odkmaker.data.getTitle();
        if ($.isBlank(htitle) || (htitle === $('h1').text())) htitle = null;
        var isCurrentForm= (odkmaker.data.currentForm!=null);
        //alert("isCurrentForm "+isCurrentForm);

        var xxx={
            title: $('h1').text(),
            isNewSecureForm: $('#formProperties_isNewSecureForm').val(),
        ...
        if(isCurrentForm){
         xxx.metadata.pdf=odkmaker.data.currentForm.pdf
         xxx.metadata.sdf=odkmaker.data.currentForm.sdf
        }
        //console.log("metadata pdf ==="+JSON.stringify(xxx.metadata.pdf));
        return xxx;
    };
    ...
    var loadOne = odkmaker.data.loadOne = function(control, $parent)
    {
        ...
        if(typeof(control.sens)!="undefined"){
        	properties.sens=control.sens
        }

        console.log(TAG+"Loading Control"+JSON.stringify(control));
        //console.log(TAG+"Loading Properties "+JSON.stringify(properties));
    ...
    odkmaker.data.load = function(formObj)
    {
        ...
        if(formObj.isNewSecureForm==1){
        	$('#secInfo').text("Securing Form");
        }else{
        	$('#secInfo').text("");
        }
        $('h1').text(formObj.title);

        $('#formProperties_isNewSecureForm').val(formObj.isNewSecureForm);
    ...
    var isStaticAttribute=function(attributeName,pdf_form_attribues){
      var psparams=[]
      for(var i=0;i<pdf_form_attribues.length;i++){
        var pdf_attribute=pdf_form_attribues[i];
        var pdf_a_name=pdf_attribute.name;
        if(pdf_a_name==attributeName){
          var myParams=pdf_attribute.psparams
    ...
    var extractConds=function(conds){
      var myItems=[]
      for(var i=0;i<conds.length;i++){
        var cond=conds[i];
        var c_op=cond.op;
        var c_level=cond.level;
        var c_value1=cond.value1;
        var c_value2=cond.value2;
        var pTag={
          name:"cond",
          attrs:{
            "op":c_op,
        ...
        console.log("added condition="+c_op);
        myItems.push(pTag)
      }
      return myItems
    }
    var extractParams=function(psparams){
      var myItems=[]
      for(var i=0; i<psparams.length;i++){
        var param=psparams[i];
        var p_name=param.name;
        var p_aspect=param.aspect;
        var conds=param.conds;
        var pTag={
          name:"param",
          attrs:{
            "name":p_name,
        ...
        if(typeof(conds) !="undefined"){
            //console.log("found conditions " +conds.length)
            pTag.children=extractConds(conds)
        }
        myItems.push(pTag)
      }
      return myItems
    ...
    var parseSensitivity=function(control,xpath,sensitivity,pdf_form_attribues){

      if(typeof(control.sens) !="undefined"){
        var a_name=control.name
        console.log("attribute type="+control.type)
        var a_level=control.sens.level
        var sTag = {
            name: a_name,
            attrs: {
                'ref': xpath + a_name,
        ...
        if(typeof(pdf_form_attribues)!="undefined"){
          var psparams=isStaticAttribute(a_name,pdf_form_attribues)
          if(psparams.length>0)
          {
            console.log("Is static " +a_name)
            sTag.children=extractParams(psparams)
          }
          sensitivity.children.push(sTag)
        }
      }
    };

    var parseSDf=function(sdfData){
      var myItems=[]
      var levels=["public","confidential","critical"]
      for(var ll=0;ll<levels.length;ll++){
        var myLevelName=levels[ll]
        var myLevel=sdfData[myLevelName]
        if(typeof(myLevel)!="undefined"){
          var tag={
            "name":myLevelName,
            attrs:{
              "encryption":myLevel.encryption,
              "masking":myLevel.masking,
              "accesscontrol":myLevel.accesscontrol,
              "hashing":myLevel.hashing,
              "password":myLevel.password,
              "anonymise":myLevel.anonymise,
              "storagearea":myLevel.storagearea,
              "deidentification":myLevel.deintification
            }
          }
            myItems.push(tag)
        }
    ...
    {
       //console.log("Internal="+JSON.stringify(internal.metadata))
        ...
        var sdf
        var sdfData=internal.metadata.sdf

        var isSDFDefined= (typeof(sdfData)!="undefined")
        if(isSDFDefined){
          sdf={
            name:"h:sdf",
            attrs:{"profle":sdfData.name},
            children:[]
          };
        };
        ...
        var pdf_form_attribues;
        var pdf=internal.metadata.pdf
        if(typeof(pdf)!="undefined"){
            var pdf_form_attribues=  internal.metadata.pdf.form.attributes
        }else{
            pdf_form_attribues=pdf
        }
        ...
        _.each(internal.controls, function(control)
        {
            ...
            if(control.type=="group"){
            var gChildren=control.children
              for(var i=0;i<gChildren.length;i++){
                var cx=gChildren[i]
                parseSensitivity(cx,"/data/",sensitivity,pdf_form_attribues);
              }
            }else{
              parseSensitivity(control,"/data/",sensitivity,pdf_form_attribues);
            }

        });

        //if(internal.isNewSecureForm!=""){
        //  root.children.push(sensitivity)
        //}

        if(isSDFDefined){
          root.children.push(sensitivity)
          root.children.push(sdf)
        }
    ...
    {
      //console.log("Value replacement="+value)
