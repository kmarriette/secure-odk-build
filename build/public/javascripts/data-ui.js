...
;(function($)
{
    ...
    //KKM start
    // new secure form by loading the plain forms
    var newSecureForm = function()
    {
        $('.createSecureFormDialog .modalLoadingOverlay').fadeIn();
        $.ajax({
            url: '/sformcreate/' + $('.createSecureFormDialog .formList li.selected').attr('rel'),
            dataType: 'json',
            type: 'GET',
            success: function(response, status)
            {
                odkmaker.data.load(response);
                $('.createSecureFormDialog').jqmHide();
            },
            error: function(request, status, error)
            {
                $('.createSecureFormDialog .errorMessage')
                    .empty()
                    .append('<p>Could not open the form. Please try again in a moment.</p>')
                    .slideDown();
            }
    ...
    // open previously saved secure Forms
    var openSecureForm = function()
    {
        $('.openSecureFormDialog .modalLoadingOverlay').fadeIn();
        $.ajax({
            url: '/sformopen/' + $('.openSecureFormDialog .formList li.selected').attr('rel'),
            dataType: 'json',
            type: 'GET',
            success: function(response, status)
            {
                odkmaker.data.load(response);
                $('.openSecureFormDialog').jqmHide();
            },
            error: function(request, status, error)
            {
                $('.openSecureFormDialog .errorMessage')
                    .empty()
                    .append('<p>Could not open the form. Please try again in a moment.</p>')
                    .slideDown();
            }
  ...
  //KKM end
    $(function()
    {
        ...
        $('.menu .saveLink').click(function(event)
        {
            ...
            var  url= '/form/' + odkmaker.data.currentForm.id;
            //var isNewSecureForm=odkmaker.data.currentForm.isNewSecureForm;
        ...
        //KKM start
        // modal events for createSecureForm
        var $createSecureFormDialog = $('.createSecureFormDialog');

        $createSecureFormDialog.delegate('.formList li', 'click', function(event)
        {
            event.preventDefault();
        ...
        $createSecureFormDialog.find('.openLink').click(function(event)
        {
            event.preventDefault();
            odkmaker.application.confirmDestruction(newSecureForm);

        });
        $createSecureFormDialog.delegate('.formList li', 'dblclick', function(event)
        {
            event.preventDefault();
            newSecureForm();
        });
        $createSecureFormDialog.delegate('.formList li a.deleteFormLink', 'click', function(event)
        {
            event.preventDefault();
        ...
        // modal events for Open Secure Forms
        var $openSecureFormDialog = $('.openSecureFormDialog');

        $openSecureFormDialog.delegate('.formList li', 'click', function(event)
        {
            event.preventDefault();
        ...
        $openSecureFormDialog.find('.openLink').click(function(event)
        {
            event.preventDefault();
            odkmaker.application.confirmDestruction(openSecureForm);

        });
        $openSecureFormDialog.delegate('.formList li', 'dblclick', function(event)
        {
            event.preventDefault();
            openSecureForm();
        });
        $openSecureFormDialog.delegate('.formList li a.deleteFormLink', 'click', function(event)
        {
            event.preventDefault();

            var id = $(this).closest('li').attr('rel');
            odkmaker.application.confirm('Are you absolutely sure you want to delete this form? This cannot be undone.', function() { deleteFormSecure(id); });
        });




        var deleteFormSecure = function(id)
        {
        	$openSecureFormDialog.find('.modalLoadingOverlay').fadeIn();
            ...
            $.ajax({
                ...
                success: function(response, status)
                {
                    ...
                    $('.openSecureFormDialog .errorMessage').empty();
                },
                error: function()
                {
                    $('.openSecureFormDialog .errorMessage').empty().append('<p>Something went wrong when trying to delete ' +
                        'that form. Please try again later.');
                },
                complete: function()
                {
                	$openSecureFormDialog.find('.modalLoadingOverlay').fadeOut();
                }
            });
        };
        //KKM End
        ...
        // loadSDF file start
        var loadSDFFileValid = false;
        var loadSDFFileUploader = new AjaxUpload('loadFileSDFChooseLink', {
            action: '/binary/load',
            name: 'file',
            autoSubmit: false,
            responseType: 'json',
            onChange: function(fileName, ext)
            {
                $('#loadSDFFile_name').val(fileName);

                loadSDFFileValid = true //_.isString(ext) && !!ext.match(/^odkbuild$/i);
                $('.loadSDFLocallyDialog .errorMessage')
                    .text('You must choose an ODK Build form (.odkbuild) file!')
                    .toggle(!loadSDFFileValid);
            },
            onSubmit: function() { return loadSDFFileValid; },
            onComplete: function(fileName, response)
            {
                $('#loadSDFFile_name').val('');
                alert("loading");
                // we've loaded a file, but we don't want it to be canonical
                // they'll have to save it to get it upstream.
                dataNS.currentForm = null;
                ...
                $.toast($.h(fileName) + ' has been loaded, but it is unsaved. Please go to ' +
                                        'File &raquo; Save if you wish to save it.');
                $('.loadSDFLocallyDialog').jqmHide();
            }
        });

        $('.loadSDFLocallyDialog .loadSDFFileLink').click(function(event)
                {

                    event.preventDefault();
                    loadSDFFileUploader.submit();
                });
        //loadSDF End
        //KKM export secure form
        $('.exportDialogSecureForm .downloadLink').click(function(event)
        {
            event.preventDefault();
