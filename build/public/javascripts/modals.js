...
;(function($)
{
    var handlers = {
        ...
        //KKM start
        createSecureFormDialog: function($dialog)
        {
            ...
            odkmaker.auth.verify(function()
            {
                ...
                _.each(sortedForms, function(formObj)
                {
                    $list.append('<li rel="' + formObj.id + '" class="newSecureForm">' + $.h(formObj.title) +
                      '<a href="#delete" class="icon deleteFormLink">delete</a></li>');
                });
                if (sortedForms === 0)
                {
        ...
        openSecureFormDialog: function($dialog)
        {
            ...
            odkmaker.auth.verify(function()
            {
                ...
                var sortedForms = odkmaker.auth.currentUser.sforms.slice(0).sort(function(a, b) {
                    return a.title.localeCompare(b.title);
                });
        ...
        loadSDFLocallyDialog: function($dialog)
        {
            $dialog.find('.errorMessage').hide();
            $dialog.find('#loadSDFile_name').val('');
            console.log("file");
        },
        //KKM end
        ...
        //KKM Start
        exportDialogSecureForm: function($dialog)
        {
