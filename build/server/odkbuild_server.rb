...
require './server/lib/filereader'

require './server/model/user'
require './server/model/form'
require './server/model/secureForm'
...
class OdkBuild < Sinatra::Application
  ...
  get '/form/:form_id' do
    ...
    puts form.data.to_json
    return form.data.to_json
  end


 get '/sformcreate/:form_id' do
 	puts "reading sformcreate/id"
    user = env['warden'].user
    ...
    return error_permission_denied if form.owner != user
    formData = form.data
   	formData["isNewSecureForm"]=1

    pdfData= FileReader.readPDF()
    ccName=FileReader.readProfileName(pdfData)
    sdfData=FileReader.readSDF()
    sdfDataCC= FileReader.contextSecurityMechanisms(sdfData,ccName)
    formData[:sdf]=sdfDataCC
    formData[:pdf]=pdfData
   	#data=form.staticClassify(formData)
    data=FileReader.staticClassify(formData)
   	#puts "secure data  " +data.to_s
   	return data.to_json
  end

 get '/sformopen/:form_id' do
 	puts  "reading sformopen/id"
    user = env['warden'].user

    form = SecureForm.find(params[:form_id])
    ...
    #formData["pdf"]=FileReader.readPDF()

   	return formData.to_json
  end
  ...
  put '/form/:form_id' do
    ...
    form = Form.find(params[:form_id])
    if(form.nil?)
    	secureForm = SecureForm.find(params[:form_id])

       	form = Form.find_by_id(secureForm.form_id)
    end
    return error_not_found if form.nil?

    return error_permission_denied if form.owner != user

    if(request_data[:isNewSecureForm]=="1")
    	request_data[:form_id]=form.id
    	#file_name="./../public/sdf/sdf.json"
    	...
    	#request_data[:sdf]=pdf_data
    	puts "Saving Secure form "
    	secureForm = SecureForm.find_by_plain_form_id(form.id)

    	if(secureForm.blank?)
    		puts "Saving Secure form  is blank"
    		SecureForm.create request_data, user
    	else
    		puts "Updating Secure form"
    		secureForm.update request_data
    		secureForm.save
    	end
    end
 ...
 delete '/sform/:form_id' do
    user = env['warden'].user

    secureForm = SecureForm.find(params[:form_id])

    return error_not_found if secureForm.nil?
    return error_permission_denied if secureForm.owner != user
  ...
  post '/download' do
    ...
    attachment params[:filename]
     #puts "downloading"+ params[:payload]
